...
function shortTrim(answer) {
  try {
    ...
    const question = questionPart.slice(1).trim(); // "Q: " 제거
    const answerText = answerPart.length >= 2 ? answerPart.slice(1).trim() : answerPart.trim(); // "A: " 제거
...
function selectionTrim(questionString) {
  try {
    ...
    // 첫 번째 줄에서 질문 추출
    const question = parts[0].slice(1).trim(); // "Q. " 제거

    // 나머지 줄에서 선택지 추출
    const choices = [];
    for (let i = 1; i <= 4; i++) {
      // 문자열 길이 확인 후 slice
      const choice = parts[i].length >= 1 ? parts[i].slice(3) : parts[i];
      choices.push(choice.trim()); // "1. "부터 "4. "까지 제거
    }

    // 마지막 줄에서 정답 추출
    const answer = parts[5].length >= 1 ? parts[5].slice(1).trim() : parts[5].trim(); // "A. " 제거
...
function blankTrim(answer) {
  try {
    const [questionPart, answerPart] = answer.split('\n');
    const question = questionPart.slice(1).trim(); // "Q: " 제거
    const answerText = answerPart.length >= 1 ? answerPart.slice(1).trim() : answerPart.trim(); // "A: " 제거
    return { question, answerText };
  } catch (error) {
...
async function startChat() {
  ...
  try {
    ...
    let fileContent;
    try {
      fileContent = await fs.readFile('./resources/keyword.txt', 'utf-8');
    } catch (readError) {
